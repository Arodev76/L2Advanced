	...
	public void NewAccount(String[] name)
	{
		...
		try
		{
			try (Connection con = LoginDatabaseFactory.getInstance().getConnection();
				PreparedStatement offline = con.prepareStatement("SELECT `login` FROM `accounts` WHERE `login` = ?"))
			{
				...
				try (ResultSet rs = offline.executeQuery())
				{
					if (rs.next())
					{
						...
						try (Connection conGS = DatabaseFactory.getInstance().getConnection();
							Statement statement = conGS.createStatement())
						{
